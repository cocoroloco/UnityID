...
public class Parallaxer : MonoBehaviour {
	...
	public float chanceOfVerticalMovement = 0.3f; // Probabilidad de que un wood tenga movimiento vertical
	
	public GameObject coinPrefab; // Prefab de la moneda
	public float coinSpawnChance = 0.3f; // Probabilidad de que se genere una moneda
	...
	void Spawn(){
		...
		// Asignar el script de movimiento vertical con cierta probabilidad
		if (Random.value < chanceOfVerticalMovement)
		{
			if (t.gameObject.name.Contains("Wood") || t.gameObject.name.Contains("Coin") )
			{
				t.gameObject.AddComponent<PipeVerticalMovement>();
			}
		}
